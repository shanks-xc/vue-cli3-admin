...
let removePending = (config, cancleHandle) => {
  ...
  try {
    ...
    if (_config.url.indexOf(_config.baseURL) === -1 || _config.url.indexOf(_config.baseURL) !== 0) {
      if (process.env.NODE_ENV === 'production') {
        // if (true) {
        _config.baseURL.substr(_config.baseURL.length - 1, 1) !== '/' && (_config.url = '/' + _config.url)
      } else {
        _config.url = _config.url.indexOf('/') === 0 ? _config.url.substr(1, _config.url.length) : _config.url
      }
      _config.url = _config.baseURL + _config.url
    }
    if (_config.method === 'post' && typeof _config.data === 'string' && _config.headers['Content-Type'] === 'application/json') {
    ...
    let _con = {
      ...
      param: _config.method === 'get' || _config.method === 'delete' ? _config.params : _config.data
    }
 ...
 *
 */
let noneHeaderArray = ['bosszuul/excel/import/getTask', 'bosszuul/excel/import/startTask', 'bosszuul/excel/import/sign', 'bosszuul/excel/import/createTask']
...
instance.interceptors.response.use((response) => {
  ...
  pending = []
  let url = ['product/operateProduct/batchOut', 'product/operateProduct/batchPutaway', 'product/operateProduct/batchCopyProduct']
  if (error.response) {
